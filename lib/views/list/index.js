...
function getOpListener (self, listener) {
  ...
  return { 
    to: function trigger (property) {
      ...
      if (self.visible) {
        self.application.animate({ update: update });
      } else {
        update();
      }
    }
...
BaseView.extend(ListView, {
  ...
  willRender: function () {
    ...
    this.bind("modelViewClass", getOpListener(this, this._onModelViewFactoryChange)).now();
    this.bind("modelViewFactory", getOpListener(this, this._onModelViewFactoryChange)).now();
    this.bind("filter", getOpListener(this, this._onFilterChange)).now();
    this.bind("sort", getOpListener(this, this._onSortChange)).now();
  ...
  _onSourceChange: function (newSource, oldSource) {
    ...
    if (newSource) this._sourceListeners.add(newSource.on("update", _.bind(this._onSourceUpdate, this)));
  ...
  _watchModels: function (models) {
    ...
    for (var i = models.length; i--;) {
      this._sourceListeners.add(models[i].on("change", onModelChange));
    }
  },
  ...
  _onSourceUpdate: function (sourceChanges) {
    ...
    if (sourceChanges.remove) this._removeChildren(sourceChanges.remove);
  },
